...
// docker config file.
func GetAuthConfig(username, password, registry string) (registry.AuthConfig, error) {
	...
	if err != nil {
		return registry.AuthConfig{}, fmt.Errorf("loading config file failed: %v", err)
	}
	...
	if !dcfg.ContainsAuth() {
		...
		if registry != "" {
			return setDefaultRegistry(registry.AuthConfig{
				ServerAddress: registry,
			}), nil
		}
	...
	if err != nil {
		return registry.AuthConfig{}, fmt.Errorf("getting credentials failed: %v", err)
	}
	...
	if registry != "" {
		...
		// see https://github.com/Ju-NXH/reg/issues/32
...
func fixAuthConfig(creds cliregistry.AuthConfig, registry string) (c registry.AuthConfig) {
...
func setDefaultRegistry(auth registry.AuthConfig) registry.AuthConfig {
